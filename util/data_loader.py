...
def get_loader_in(args, config_type='default', split=('train', 'val')):
    ...
    if args.in_dataset == "CIFAR-10":
        ...
        root = '/home/lenovo/wfc/LAPS-main/datasets/id_data/imagenet'
        # Data loading code
        if 'train' in split:
...
def get_loader_out(args, dataset=(''), config_type='default', split=('train', 'val')):
    ...
    if 'val' in split:
        ...
        if val_dataset == 'SVHN':
            ...
            val_ood_loader = torch.utils.data.DataLoader(SVHN('datasets/ood_data/svhn/images', split='test', transform=transform_test, download=False),
                                                       batch_size=batch_size, shuffle=False,
                                                        num_workers=2)
        #elif val_dataset == 'dtd':
        #    val_ood_loader = torch.utils.data.DataLoader(torchvision.datasets.ImageFolder("./datasets/ood_data/dtd".format(val_dataset),
        ...
        #    val_ood_loader = torch.utils.data.DataLoader(torchvision.datasets.ImageFolder(root="datasets/ood_data/dtd/images", transform=transform_test),
        #                                              batch_size=batch_size, shuffle=True, num_workers=2)
            ...
            val_ood_loader = torch.utils.data.DataLoader(torchvision.datasets.ImageFolder("./datasets/ood_data/sun50".format(val_dataset),
                                                          transform=config.transform_test_largescale), batch_size=batch_size, shuffle=False, num_workers=2)
        elif val_dataset == 'dtd':
            val_ood_loader = torch.utils.data.DataLoader(torchvision.datasets.ImageFolder("./datasets/ood_data/dtd".format(val_dataset),
                                                          transform=config.transform_test), batch_size=batch_size, shuffle=False, num_workers=2)
        elif val_dataset == 'iSUN':
            val_ood_loader = torch.utils.data.DataLoader(torchvision.datasets.ImageFolder("./datasets/ood_data/iSUN".format(val_dataset),
                                                          transform=config.transform_test), batch_size=batch_size, shuffle=False, num_workers=2)
        elif val_dataset == 'LSUN':
            val_ood_loader = torch.utils.data.DataLoader(torchvision.datasets.ImageFolder("./datasets/ood_data/LSUN".format(val_dataset),
                                                          transform=config.transform_test), batch_size=batch_size, shuffle=False, num_workers=2)
